...
Peerio.UI.controller('preferences', function($scope, $window) {
    ...
    $scope.preferences.unlock = function() {
        ...
        Peerio.user.getPIN(Peerio.user.username, function(PINExists) {
            ...
            Peerio.crypto.getKeyFromPIN(input, Peerio.user.username, function(keyBytes) {
                ...
                swal(document.l10n.getEntitySync('error').value, document.l10n.getEntitySync('incorrectPasswordOrPIN').value);

            });
        });
    };
    $scope.preferences.canSeePassphrase = function() {
        return !!Peerio.user.passphrase;
    }
    $scope.preferences.showPassphrase = function(){
        swal(document.l10n.getEntitySync('passphrase').value,Peerio.user.passphrase);
    }
