...
(function() {
    ...
    Peerio.user.setPIN = function(PIN, username, callback) {
        ...
        Peerio.crypto.getKeyFromPIN(PIN, username, function(keyBytes) {
            ...
            db.get('PIN', function(err, data) {
                ...
                if (Peerio.user.passphrase) {
                    var encryptedPP = Peerio.crypto
                        .secretBoxEncrypt(nacl.util.decodeUTF8(Peerio.user.passphrase), keyBytes);

                    db.get('PP', function(err, ppData) {
                        if (typeof(ppData) === 'object') db.remove(ppData);
                        db.put({ _id: 'PP', passphrase: encryptedPP });
                    });
                }
    ...
    Peerio.user.getPIN = function(username, callback) {
        Peerio.storage.db.get('PIN', function(err, data) {
            if (typeof(data) === 'object' && hasProp(data, 'PIN') && typeof(data['PIN']) === 'object' &&
                hasProp(data.PIN, 'ciphertext') && hasProp(data.PIN, 'nonce')) {
                Peerio.user.PIN = {
                    ciphertext: Peerio.util.objectToUint8Array(data['PIN'].ciphertext),
                    nonce: Peerio.util.objectToUint8Array(data['PIN'].nonce)
                }
            }
            Peerio.storage.db.get('PP', function(err2, data2) {
                if (typeof(data2) === 'object') {
                    Peerio.user.encryptedPP = {
                        ciphertext: Peerio.util.objectToUint8Array(data2.passphrase.ciphertext),
                        nonce: Peerio.util.objectToUint8Array(data2.passphrase.nonce)
                    };
                }
